...
export function useWorkspaceScopedActions() {
  ...
  const cloneAgent = useCallback(
    async (sourceAgentId: string, agentData: any) => {
      if (!isReady || !currentWorkspaceId) {
      ...
      try {
        ...
        if (result.success) {
          ...
          setAgentsLoading({
            isLoading: false,
            error: result.error || "Failed to clone agent",
          });
        }
      } catch (error) {
  ...
  const getAgentById = useCallback(
    async (agentId: string) => {
      ...
      try {
        ...
        console.error(
          "[useWorkspaceScopedActions] Error in getAgentById:",
          error,
        );
  ...
  const getAgentVersions = useCallback(
    async (parentAgentId: string) => {
      ...
      try {
        const result = await executeWorkflow<Agent[]>(
    ...
    async (agentId: string) => {
      ...
      try {
        ...
        if (result.success) {
          ...
          setAgentsLoading({
            isLoading: false,
            error: result.error || "Failed to publish agent",
          });
        }
      } catch (error) {
        setAgentsLoading({
  ...
  const archiveAgent = useCallback(
    async (agentId: string) => {
      ...
      let result;
      try {
        result = await executeWorkflow<{ agent: Agent }>(
        ...
        if (result.success) {
          ...
          setAgentsLoading({
            isLoading: false,
            error: result.error || "Failed to archive agent",
          });
        }
      } catch (error) {
        setAgentsLoading({
  ...
  const createTask = useCallback(
    async (taskData: any) => {
      ...
      try {
        ...
        console.error(
          "[useWorkspaceScopedActions] Error in createTask:",
          error,
        );
  ...
  const createTeam = useCallback(
    async (teamData: { name: string; description?: string; icon?: string }) => {
      if (!isReady || !currentWorkspaceId) {
    ...
    async (id: string) => {
      ...
      try {
        ...
        if (result.success) {
          ...
          setMcpServersLoading({
            isLoading: false,
            error: result.error || "Failed to delete MCP server",
          });
        }
      } catch (error) {
        setMcpServersLoading({
  ...
  const getMcpServerById = useCallback(
    async (id: string) => {
      ...
      try {
        const result = await executeWorkflow<McpServer>(
  ...
  const openaiServer = useMemo(
    () =>
      mcpServers.find((s) => s.server_label?.toLowerCase() === "openai") ??
      null,
    [mcpServers],
  );
  const hasWorkspaceOpenAIToken = useMemo(
