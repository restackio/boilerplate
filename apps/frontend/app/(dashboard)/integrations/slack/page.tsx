...
export default function SlackIntegrationPage() {
  ...
  // here with ?slack_install=success|already_connected_elsewhere|failed|storage_failed
  // after handling /slack/oauth/callback. We show a toast, refresh state, and
  // strip the query params so a manual refresh doesn't re-toast.
  const oauthHandledRef = useRef(false);
  useEffect(() => {
    if (oauthHandledRef.current) return;
    const status = searchParams.get("slack_install");
    if (!status) return;
    ...
    if (status === "success") {
      toast({
        ...
        description: team
          ? `${team} is now connected to this workspace.`
          : "Your Slack workspace is now connected.",
      });
      fetchIntegration();
    } else if (status === "already_connected_elsewhere") {
      toast({
        title: "Slack workspace already connected",
        description: team
          ? `${team} is connected to a different Restack workspace. Ask its admin to disconnect it first, then try again.`
      ...
      toast({
        ...
        description: reason
          ...
          : "Slack rejected the install. Please try again.",
        variant: "destructive",
      });
