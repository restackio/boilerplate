...
export function TaskSubtasksList({
  ...
  const { subtasks, statusCounts } = useMemo(() => {
    ...
    const completed = parsedSubtasks.filter(
      (s) => s.status === "completed",
    ).length;
    const inProgress = parsedSubtasks.filter(
  ...
  return (
    <div className="max-w-4xl mx-auto border border-border/40 bg-muted/90 p-2 rounded-lg space-y-2">
      ...
      >
        ...
        {failedCount > 0 && (
          <span className="text-sm text-red-600">• {failedCount} failed</span>
        )}
      </div>
