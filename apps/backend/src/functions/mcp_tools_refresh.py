...
async def mcp_session_init(
    ...
    try:
        async with aiohttp.ClientSession() as session:
            ...
            request_headers.setdefault("MCP-Protocol-Version", "2025-03-26")
            ...
            async with session.post(function_input.server_url,
                                  json=init_payload,
                                  headers=request_headers,
                                  timeout=10) as init_response:
                    ...
                    if "text/event-stream" in content_type:
                        ...
                        async for line in init_response.content:
                            ...
                            if line_text.startswith("data: "):
                                ...
                                try:
                                    init_data = json.loads(current_data)
...
async def mcp_tools_list(
    ...
    try:
        async with aiohttp.ClientSession() as session:
            ...
            request_headers.setdefault("Accept", "application/json, text/event-stream")
            request_headers.setdefault("MCP-Protocol-Version", "2025-03-26")
            request_headers["Mcp-Session-Id"] = function_input.session_id
            ...
            async with session.post(function_input.mcp_endpoint,
                                  json=tools_payload,
                                  headers=request_headers,
                                  timeout=10) as tools_response:
                                ...
                                try:
                                    ...
                                    if isinstance(tools_data, dict) and "result" in tools_data:
                                        ...
                                        if "tools" in result and isinstance(result["tools"], list):
                                            tool_names = [tool.get("name", "") for tool in result["tools"] if tool.get("name")]
                        ...
                        async for line in tools_response.content:
                            ...
                            if line_text.startswith("data: "):
                                ...
                                try:
                                    tools_data = json.loads(current_data)
                                    if isinstance(tools_data, dict) and "result" in tools_data:
                                        tools_info = tools_data["result"].get("tools", [])
                                        tool_names = [tool.get("name", "") for tool in tools_info if "name" in tool]
