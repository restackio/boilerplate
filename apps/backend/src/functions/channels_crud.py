...
async def channel_route_event(
    ...
    async for db in get_async_db():
        try:
            ...
            # relax that to allow multiple agents per channel, switch this to
            # ``.order_by(Channel.created_at.desc()).limit(1)`` (or fan out to
            # all matching agents) instead of ``scalar_one_or_none``.
