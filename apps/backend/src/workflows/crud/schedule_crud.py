...
class FrontendScheduleCreateInput(BaseModel):
    ...
    schedule_spec: dict[str, Any] = Field(
        ...,
        description="Raw schedule specification from frontend",
    )
...
class FrontendScheduleEditInput(BaseModel):
    ...
    task_id: str = Field(..., min_length=1)
    schedule_spec: dict[str, Any] = Field(
        ..., description="New schedule specification"
    )
...
class ScheduleCreateWorkflow:
    ...
    @workflow.run
    async def run(
        self, workflow_input: FrontendScheduleCreateInput
    ) -> ScheduleOutput:
        log.info("ScheduleCreateWorkflow started")
        try:
            # Convert frontend format to unified format
            unified_spec = (
                UnifiedScheduleSpec.from_frontend_format(
...
class ScheduleUpdateWorkflow:
    ...
    @workflow.run
    async def run(
        self, workflow_input: FrontendScheduleUpdateInput
    ) -> ScheduleOutput:
...
def _parse_interval_to_seconds(interval_str: str) -> int:
    """Parse interval string to seconds for protobuf Duration format."""
...
def _convert_intervals_to_protobuf(
    intervals: list,
) -> list[dict[str, str]]:
    """Convert intervals to protobuf format."""
    protobuf_intervals = []
    for interval in intervals:
        seconds = _parse_interval_to_seconds(interval.every)
        protobuf_intervals.append({"interval": f"{seconds}s"})
    return protobuf_intervals


def _convert_calendars_to_protobuf(calendars: list) -> list[dict]:
    """Convert calendars to structured calendar protobuf format."""
    return [
        {
            "dayOfWeek": [
                {
...
def _convert_unified_spec_to_protobuf(
    unified_spec: UnifiedScheduleSpec,
) -> dict:
    """Convert unified schedule spec to protobuf JSON format."""
    spec_dict = {}
    ...
    if unified_spec.intervals:
        spec_dict["interval"] = _convert_intervals_to_protobuf(
            unified_spec.intervals
        )

    # Handle calendars (structured calendar format)
    if unified_spec.calendars:
        spec_dict["structuredCalendar"] = (
            _convert_calendars_to_protobuf(unified_spec.calendars)
        )
...
class ScheduleEditWorkflow:
    ...
    @workflow.run
    async def run(
        self, workflow_input: FrontendScheduleEditInput
    ) -> ScheduleOutput:
        ...
        try:
            ...
            spec_dict = _convert_unified_spec_to_protobuf(
                unified_spec
            )
...
class ScheduleControlWorkflow:
    ...
    @workflow.run
    async def run(
        self, workflow_input: ScheduleControlInput
    ) -> ScheduleOutput:
        log.info(
            f"ScheduleControlWorkflow started - Action: {workflow_input.action}"
        )
        log.info(
            f"Input - Task ID: {workflow_input.task_id}, Schedule ID: {workflow_input.schedule_id}"
        )

        try:
            action = workflow_input.action
            reason = (
                workflow_input.reason
                or f"{action.capitalize()}d from the backend"
            )
            ...
            if workflow_input.task_id:
                ...
                task_id = None
                log.info(
                    f"Using direct schedule ID: {schedule_id}"
                )
            ...
            return ScheduleOutput(
                ...
                restack_schedule_id=db_result.get(
                    "restack_schedule_id"
                ),
            )
