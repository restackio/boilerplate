...
class McpToolsListWorkflow:
    ...
    async def _resolve_server_info(
        ...
        if not workflow_input.server_url and not workflow_input.mcp_server_id:
            return (None, False, "Either server_url or mcp_server_id must be provided")

        server_url = workflow_input.server_url
        is_local = workflow_input.local

        if (server_url == "placeholder" or not server_url) and workflow_input.mcp_server_id:
            ...
            if server_result and server_result.mcp_server:
                ...
                log.info(f"Resolved MCP server - local: {is_local}, server_url: {server_url}")
            else:
                return (None, False, f"Could not resolve MCP server {workflow_input.mcp_server_id}")
    ...
    async def _get_auth_headers(
        ...
        if workflow_input.workspace_id and workflow_input.mcp_server_id:
            try:
                ...
                if token:
                    ...
                    log.info("Using default OAuth token for MCP server authentication")
                else:
                    log.info("No default token found for MCP server")
            except (ValueError, TypeError, AttributeError) as e:
                log.warning(f"Failed to get OAuth token for MCP server: {e}")
        elif workflow_input.workspace_id:
            log.info("Workspace ID provided but no MCP server ID - cannot use default token authentication")

        return headers

    def _convert_tools_to_mcp_objects(self, tools_with_descriptions: list) -> list:
    ...
    async def _list_tools_with_session(
        self, session_init_result: Any, server_url: str, headers: dict
    ) -> McpToolsListOutput:
        """List tools using session-based approach."""
        log.info(f"Server requires session management (session_id: {session_init_result.session_id})")
        ...
        if not tools_list_result.success:
            log.error(f"Session-based tools list retrieval failed: {tools_list_result.error}")
            error_message = self._enhance_auth_error(tools_list_result.error or "")
            return McpToolsListOutput(success=False, tools_list=[], error=error_message)
    ...
    async def _list_tools_direct(
        ...
        log.info("Server does not require session management, using direct tool listing")
        ...
        if not direct_result.success:
            log.error(f"Direct tools list retrieval failed: {direct_result.error}")
            error_message = self._enhance_auth_error(direct_result.error or "")
            return McpToolsListOutput(success=False, tools_list=[], error=error_message)
    ...
    async def run(
        ...
        try:
            ...
            server_url, is_local, error = await self._resolve_server_info(workflow_input)
            if error:
                return McpToolsListOutput(success=False, error=error)

            log.info(f"Listing tools for server - local: {is_local}, server_url: {server_url}")
            ...
            log.info("Attempting MCP initialization to check session requirements")
            ...
            return await self._list_tools_direct(server_url, headers, is_local)

        except (ValueError, TypeError, RuntimeError, AttributeError, ConnectionError, OSError) as e:
            error_message = f"Error during mcp_tools_list workflow: {e}"
            log.error(error_message)
            return McpToolsListOutput(
                success=False,
                tools_list=[],
