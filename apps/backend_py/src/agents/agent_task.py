...
class AgentTask:
    ...
    def __init__(self) -> None:
        ...
        self.mcp_servers = []
        self.last_response_id = None  # Track response ID for conversation continuity

    def _add_conversation_item(self, item_type: str, content: str = None, item_id: str = None, item_data: dict = None, timestamp: str = None):
        ...
        if item_type == "tool-call" and item_data:
            conversation_item.update({
                ...
                "status": "completed" if item_data.get("output") else "in-progress"
            })
        elif item_type == "tool-list" and item_data and item_data.get("tools"):
            tool_names = [tool.get("name") for tool in item_data.get("tools", [])]
            conversation_item["content"] = f"Available tools: {', '.join(tool_names)}"
            ...
            conversation_item["content"] = f"Approval required for tool: {item_data.get('name')}"
            if item_data.get("arguments"):
                conversation_item["content"] += f" with arguments: {item_data.get('arguments')}"
    ...
    def state_response(self):
        ...
        if not isinstance(self.conversation_items, list):
            log.warning("conversation_items is not a list, returning empty array")
            return []
        ...
        for item in self.conversation_items:
            ...
            if item_id not in seen_ids:
                ...
                log.warning(f"Removing duplicate item from state_response: {item_id}")
        
        log.info(f"state_response returning {len(deduplicated_items)} items (removed {len(self.conversation_items) - len(deduplicated_items)} duplicates)")
        ...
        try:
            ...
            for message in messages_event.messages:
                self._add_conversation_item(
                    ...
                    item_data={"role": message.role}  # Keep original role for backward compatibility
                )
            ...
            if not self.mcp_servers:
                log.info("Loading MCP servers configuration on first message")
                ...
                for agent_mcp_server in agent_mcp_servers_result.agent_mcp_servers:
                    ...
                    require_approval_data = McpRequireApproval.model_validate(mcp_server.require_approval).model_dump()
                ...
                log.info(f"Loaded {len(mcp_servers)} MCP servers for agent {self.agent_id}")
            ...
            try:
                ...
                if self.last_response_id:
                    llm_input.previous_response_id = self.last_response_id
                ...
                if completion.response_id:
                    ...
                    log.info(f"Updated last_response_id: {self.last_response_id}")
                ...
                if completion.final_response and completion.final_response.get("output"):
                    ...
                    for output_item in completion.final_response["output"]:
                        ...
                        if output_item.get("type") == "mcp_approval_request":
                            approval_id = output_item.get("id")
                            if approval_id and response_id:
                                self.pending_approval_requests[approval_id] = {
                                    "response_id": response_id,
                                    "approval_request": output_item
                                }
                                log.info(f"Stored MCP approval request: {approval_id} for response: {response_id}")
                        ...
                        elif output_item.get("type") == "mcp_call":
                            self._add_conversation_item(
                                item_type="tool-call",
                                item_id=output_item.get("id"),
                        ...
                        elif output_item.get("type") == "mcp_list_tools":
                            self._add_conversation_item(
                                item_type="tool-list",
                                item_id=output_item.get("id"),
                                item_data=output_item
                        ...
                        elif (output_item.get("type") == "message" and 
    ...
    @agent.event
    async def mcp_approval(self, approval_event: McpApprovalEvent) -> dict:
        """Handle MCP approval/denial events"""
        log.info(f"Received MCP approval: {approval_event.approvalId} - {'approved' if approval_event.approved else 'denied'}")
        
        # Find the pending approval request
        approval_id = approval_event.approvalId
        if approval_id not in self.pending_approval_requests:
            log.error(f"No pending approval request found for ID: {approval_id}")
            return {
        ...
        try:
            ...
            approval_data = self.pending_approval_requests[approval_id]
            ...
            log.info(f"MCP approval response completion: {completion}")
            
            # Update response ID for future conversation continuity
            if completion.response_id:
                self.last_response_id = completion.response_id
                log.info(f"Updated last_response_id after approval: {self.last_response_id}")
            
            # Update the existing approval request status in conversation items
            for item in self.conversation_items:
                if (item.get("type") == "mcp-approval-request" and 
                    item.get("id") == approval_id):
                    item["status"] = "completed" if approval_event.approved else "failed"
