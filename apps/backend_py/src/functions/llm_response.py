...
@function.defn()
async def llm_response(  # noqa: C901, PLR0912
    function_input: LlmResponseInput,
) -> LlmResponseOutput:
    try:
        log.info(
            "llm_response function started",
        ...
        log.info(
            "mcp_servers", mcp_servers=function_input.mcp_servers
        )
        ...
        if function_input.previous_response_id:
            ...
            if function_input.approval_response:
                ...
                input_data = [
                    function_input.messages[-1].model_dump()
                ]
            else:
                input_data = []
        else:
        ...
        log.info(
            "llm_response function completed",
        ...
        if final_response and hasattr(
            final_response, "model_dump"
        ):
            final_response = final_response.model_dump()
        elif final_response and hasattr(
            final_response, "__dict__"
        ):
        ...
        return LlmResponseOutput(
            ...
            usage=usage,
            has_completion=response_data.get(
