...
class AgentMcpServersReadByAgentWorkflow:
    ...
    @workflow.run
    async def run(
        self, workflow_input: AgentMcpServerGetByAgentInput
    ) -> AgentMcpServerListOutput:
...
class AgentMcpServersCreateWorkflow:
    ...
    @workflow.run
    async def run(
        self, workflow_input: AgentMcpServerCreateInput
    ) -> AgentMcpServerSingleOutput:
        ...
        try:
            ...
            error_message = (
                f"Error during agent_mcp_servers_create: {e}"
            )
            log.error(error_message)
            raise NonRetryableError(message=error_message) from e
...
class AgentMcpServersUpdateWorkflow:
    ...
    @workflow.run
    async def run(
        self, workflow_input: AgentMcpServerUpdateInput
    ) -> AgentMcpServerSingleOutput:
        ...
        try:
            ...
            error_message = (
                f"Error during agent_mcp_servers_update: {e}"
            )
            log.error(error_message)
            raise NonRetryableError(message=error_message) from e
...
class AgentMcpServersDeleteWorkflow:
    ...
    @workflow.run
    async def run(
        self, workflow_input: AgentMcpServerIdInput
    ) -> AgentMcpServerDeleteOutput:
        ...
        try:
            ...
            error_message = (
                f"Error during agent_mcp_servers_delete: {e}"
            )
            log.error(error_message)
            raise NonRetryableError(message=error_message) from e
...
class AgentMcpServersGetByIdWorkflow:
    ...
    async def run(
        ...
        try:
            ...
            error_message = (
                f"Error during agent_mcp_servers_get_by_id: {e}"
            )
            log.error(error_message)
            raise NonRetryableError(message=error_message) from e
