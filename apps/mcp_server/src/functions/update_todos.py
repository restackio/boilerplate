...
class TodoItem(BaseModel):
    ...
    content: str = Field(..., description="Short description of the todo (1 sentence)")
...
class UpdateTodosFunctionInput(BaseModel):
    ...
    temporal_agent_id: str | None = Field(default=None, description="Temporal/Restack workflow ID")
    temporal_run_id: str | None = Field(default=None, description="Temporal/Restack run ID")


class UpdateTodosFunctionOutput(BaseModel):
    """Output from the update todos function."""
    success: bool = Field(..., description="Whether the operation succeeded")
    todos: list[TodoItem] = Field(..., description="Current state of all todos (updated list will also appear in your message context)")
    message: str = Field(..., description="Progress summary (X/Y completed, N in progress)")
...
async def update_todos(
    ...
    """
    try:
        log.info("update_todos function called", extra={"input": function_input.model_dump()})

        # Validate required context from LLM
        if not function_input.temporal_agent_id:
            raise NonRetryableError(message="temporal_agent_id is required")  # noqa: TRY301

        log.info(f"Sending todo_update event to agent workflow {function_input.temporal_agent_id}")

        # Convert TodoItem objects to dicts for serialization
        todos_dicts = [todo.model_dump() for todo in function_input.todos]
        ...
        log.info("update_todos event sent successfully", extra={"result": result})

        # Extract result from agent event response and convert back to TodoItems
        if result and isinstance(result, dict):
            result_todos = result.get("todos", [])
            todos_list = [TodoItem(**todo) if isinstance(todo, dict) else todo for todo in result_todos]
            return UpdateTodosFunctionOutput(
                success=result.get("success", True),
                todos=todos_list,
                message=result.get("message", "Todos updated successfully")
            )
        return UpdateTodosFunctionOutput(
            success=True,
            todos=[],
