...
class SlackMcpApprovalInput(BaseModel):
    ...
    tool_parameters: dict[str, Any] = Field(
        ..., description="Parameters to be used"
    )
    ...
    try:
        ...
        async with httpx.AsyncClient() as client:
            ...
            if data.get("ok"):
                ...
                logger.info(
                    "Sent MCP approval request to Slack for %s",
                    function_input.tool_name,
                )
                return SlackMcpApprovalOutput(
                    success=True, message_ts=message_ts
                )
            error_msg = data.get("error", "Unknown error")
            logger.error(
                "Failed to send MCP approval to Slack: %s",
                error_msg,
            )
            return SlackMcpApprovalOutput(
        ...
        logger.exception(
            "Error sending MCP approval request to Slack"
        )
        return SlackMcpApprovalOutput(success=False, error=str(e))
