...
class SlackNotificationInput(BaseModel):
    ...
    text: str = Field(
        ...,
        description="Message text (fallback for notifications)",
    )
    ...
    user_id: str | None = Field(
        None,
        description="If provided, sends DM to this user instead",
    )
        ...
        async with httpx.AsyncClient() as client:
            ...
            if function_input.user_id:
                ...
                if not dm_data.get("ok"):
                    msg = f"Failed to open DM: {dm_data.get('error')}"
...
async def update_slack_message(
    ...
    try:
        async with httpx.AsyncClient() as client:
            ...
            if data.get("ok"):
                logger.info(
                    "Updated Slack message %s",
                    function_input.message_ts,
                )
                return SlackNotificationOutput(
                    success=True,
                    message_ts=data.get("ts"),
...
async def add_slack_reaction(
    ...
    try:
        async with httpx.AsyncClient() as client:
            ...
            if (
                data.get("ok")
                or data.get("error") == "already_reacted"
            ):
