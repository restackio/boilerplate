...
class SlackListCreateInput(BaseModel):
    ...
    channel_id: str = Field(
        ..., description="Channel to create list in"
    )
    name: str = Field(
        ..., description="List name (e.g., 'Open Tasks')"
    )
    description: str | None = Field(
        None, description="List description"
    )
    ...
    try:
        async with httpx.AsyncClient() as client:
            ...
            if data.get("ok"):
                ...
                logger.info(
                    "Created Slack list %s in channel %s",
                    list_id,
                    function_input.channel_id,
                )
                return SlackListOutput(
                    success=True, list_id=list_id
                )
            error = data.get("error", "Unknown error")
            logger.error("Failed to create Slack list: %s", error)
            return SlackListOutput(success=False, error=error)

    except Exception as e:
        logger.exception("Error creating Slack list")
        return SlackListOutput(success=False, error=str(e))
...
class SlackListItemInput(BaseModel):
    ...
    assignee_id: str | None = Field(
        None, description="User ID to assign to"
    )
    due_date: str | None = Field(
        None, description="Due date (ISO 8601)"
    )
    ...
    try:
        async with httpx.AsyncClient() as client:
            ...
            if function_input.description:
                payload["description"] = (
                    function_input.description
                )

            if function_input.assignee_id:
                payload["assignee_id"] = (
                    function_input.assignee_id
                )
            ...
            if data.get("ok"):
                ...
                logger.info(
                    "Added item %s to Slack list %s",
                    item_id,
                    function_input.list_id,
                )
                return SlackListOutput(
                    success=True, list_id=item_id
                )
            error = data.get("error", "Unknown error")
            logger.error(
                "Failed to add Slack list item: %s", error
            )
            return SlackListOutput(success=False, error=error)
...
class SlackListItemUpdateInput(BaseModel):
    ...
    completed: bool | None = Field(
        None, description="Mark as completed"
    )
    assignee_id: str | None = Field(
        None, description="New assignee"
    )
...
async def update_slack_list_item(
    ...
    try:
        async with httpx.AsyncClient() as client:
            ...
            if data.get("ok"):
                logger.info(
                    "Updated Slack list item %s",
                    function_input.item_id,
                )
                return SlackListOutput(success=True)
            error = data.get("error", "Unknown error")
            logger.error(
                "Failed to update Slack list item: %s", error
            )
            return SlackListOutput(success=False, error=error)
