...
class TodoItem(BaseModel):
    ...
    order_index: int = Field(default=0, description="Order position in the list")
...
class TodoUpdateInput(BaseModel):
    ...
    metadata: dict[str, Any] | None = Field(default=None, description="Optional metadata update")


class TodoUpdateOutput(BaseModel):
    """Output from updating a todo."""
    success: bool = Field(..., description="Whether the update succeeded")
    message: str = Field(..., description="Status message with progress summary")


@function.defn()
async def todo_update_status(function_input: TodoUpdateInput) -> TodoUpdateOutput:
