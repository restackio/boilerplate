...
export function FilterValueOptionDisplay<TData>({
  ...
  if (selected.length === 0) return <Ellipsis className="size-4" />;
  ...
  if (selected.length === 1) {
    ...
    if (!selectedOption) return <Ellipsis className="size-4" />;
...
export function FilterValueDateDisplay<TData>({
  ...
  if (!startDate || !endDate) return <Ellipsis className="size-4" />;
...
export function FilterValueTextDisplay<TData>({
  ...
  if (filter.values.length === 0 || !firstValue || firstValue.trim() === "")
    ...
    (type: "single" | "range") => {
      ...
      if (type === "single") {
        if (firstValue !== undefined) {
          newValues = [firstValue]; // Keep the first value for single mode
        }
      } else if (!minMax) {
        if (firstValue !== undefined) {
          newValues = createNumberRange([firstValue, secondValue ?? 0]);
        }
      } else if (firstValue !== undefined) {
        newValues =
          firstValue - minMax[0] < minMax[1] - firstValue
            ? createNumberRange([firstValue, minMax[1]])
            : createNumberRange([minMax[0], firstValue]);
      }
          ...
          <div className="flex flex-col w-full">
            ...
            >
              ...
              <TabsContent value="single" className="flex flex-col gap-4 mt-4">
                {minMax && (
                  <Slider
                    value={values[0] !== undefined ? [values[0]] : [0]}
                ...
                <div className="flex items-center gap-2">
                  ...
                  <DebouncedInput
                    id="single"
                    type="number"
                    value={values[0] !== undefined ? values[0].toString() : "0"}
