...
- **Optional: add remote MCP integrations.** If the user needs web search, external APIs, or other capabilities beyond the default pipeline tools: (1) use `searchremotemcpdirectory` with a query (e.g. "search", "github") to find relevant MCPs; (2) use `createintegrationfromremotemcp` with workspace_id from meta_info and the chosen entry's server_url and server_label; (3) use `listintegrationtools` with the returned mcp_server_id and workspace_id to get the integration's tool names; (4) for each tool name returned, call `addagenttool` with agent_id (parent or pipeline agent), tool_name, and mcp_server_id to attach the proper tools to the agent.
...
-- Build agent tools: updatetodos, createdataset, createagent, addagenttool, createview, updateview, createsubtask, searchremotemcpdirectory, createintegrationfromremotemcp, listintegrationtools.
