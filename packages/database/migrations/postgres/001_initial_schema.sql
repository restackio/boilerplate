...
CREATE TABLE IF NOT EXISTS agents (
    ...
    model VARCHAR(100) NOT NULL DEFAULT 'gpt-5.4' CHECK (length(model) > 0 AND length(model) <= 100),
...
-- Omit description from INCLUDE: long text blows past btree row size (~2704 bytes).
